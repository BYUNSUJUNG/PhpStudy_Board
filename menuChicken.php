...
<body>
    ...
    <div class="container">		
        ...
        <br><br><br>
        <?php 
            require_once("menuChickenDao.php"); // java inport와 유사함, once를 하면 한번만 읽어옴
        ...
        <?php if($totalCount>0) : ?>  
            ...
            <div class="menu">
                <a href='menuChickenView.php?num=<?= $row["num"] ?>&writer=<?= $row["writer"]?>&title=<?=$row["title"]?>&file=<?=$row["file"]?>'>
            ...
            <?php if($startPage>1) : ?>
                <a href='<?=combineUrl("menuChicken.php",0,$currentPage-MENU_NUM_PAGE_LINKS)?>' ><</a>&nbsp;
            <?php endif?>

            <?php for($i=$startPage;$i<=$endPage-1;$i++) : ?>
                <?php if($i==$currentPage) : ?>
                    <a href="<?=combineUrl('menuChicken.php',0,$i)?>"><b><?= $i?></b></a>&nbsp;
                <?php else : ?>
                    <a href="<?=combineUrl('menuChicken.php',0,$i)?>"><?= $i ?></a>&nbsp;
                <?php endif ?>
            <?php endfor ?>

            <?php if($endPage<$totalPage) : ?>
                <a href="<?=combineUrl('menuChicken.php',0,$startPage+MENU_NUM_PAGE_LINKS)?>">></a>
            <?php endif ?>
        <?php endif ?>
        <?php if($id) : ?>
            <input class="btn btn-primary" type="button" value="글쓰기" onclick="location.href='write_form.php?writeNum=3'">
        <?php endif ?>
    </div>
