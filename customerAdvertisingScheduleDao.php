...
<?php
	class customerAdvertisingScheduleDao {
		...
		function insertBoard($writer, $title, $content) { 
			try {
				$sql = "insert into advertisingSchedule(writer,title,content) values(:writer,:title,:content)";
		...
		function updateBoard($num, $writer, $title, $content) { 
			try {
				$sql = "update advertisingSchedule set writer=:writer, title=:title, content=:content where num=:num";
				$pstmt = $this->db->prepare($sql);
				$pstmt->bindValue(":num",$num,PDO::PARAM_STR);
				$pstmt->bindValue(":writer",$writer,PDO::PARAM_STR);
				$pstmt->bindValue(":title",$title,PDO::PARAM_STR);
