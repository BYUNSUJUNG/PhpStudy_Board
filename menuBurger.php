...
<body>
    ...
    <div class="container">		
        ...
        <br><br><br>
        <?php 
            require_once("menuBurgerDao.php"); // java inport와 유사함, once를 하면 한번만 읽어옴
        ...
        ?> 
            ...
            <div class="menu">
                <a href='menuBurgerView.php?num=<?= $row["num"] ?>&writer=<?= $row["writer"]?>&title=<?=$row["title"]?>&file=<?=$row["file"]?>'>
            ...
            <hr>
            <?php if($startPage>1) : ?>
                <a href='<?=combineUrl("menuBurger.php",0,$currentPage-MENU_NUM_PAGE_LINKS)?>' ><</a>&nbsp;
            <?php endif?>

            <?php for($i=$startPage;$i<=$endPage-1;$i++) : ?>
                <?php if($i==$currentPage) : ?>
                    <a href="<?=combineUrl('menuBurger.php',0,$i)?>"><b><?= $i?></b></a>&nbsp;
                <?php else : ?>
                    <a href="<?=combineUrl('menuBurger.php',0,$i)?>"><?= $i ?></a>&nbsp;
                <?php endif ?>
            <?php endfor ?>

            <?php if($endPage<$totalPage) : ?>
                <a href="<?=combineUrl('menuBurger.php',0,$startPage+MENU_NUM_PAGE_LINKS)?>">></a>
            <?php endif ?>
