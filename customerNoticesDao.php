...
<?php
	class customerNoticesDao {
		...
		function getMsg($num){ 
            try{
                $pstmt = $this->db->prepare("select * from notices where num=:num");
		...
		function increaseHits($num) { 
			try {
				$sql = "update notices set hits=hits+1 where num=:num";
				$pstmt = $this->db->prepare($sql);
				$pstmt->bindValue(":num",$num,PDO::PARAM_STR);
				$pstmt->execute(); // 실행
			} catch(PDOException $e) {
		...
		function getManyMsgs($num_page) { 
			...
			try {
				$numLine=NUM_LINES;
				$pstmt=$this->db->prepare("select * from notices order by num desc limit $num_page, $numLine");
				// bindValue 필요 없음
				$pstmt->execute();
		...
		function deleteBoard($num) { 
			try { 
				$sql = "delete from notices where num=:num";
				$pstmt = $this->db->prepare($sql);
				$pstmt->bindValue(":num",$num,PDO::PARAM_STR);
				$pstmt->execute(); // 실행
			} catch(PDOException $e) {
