...
*/
            ...
            /* select * from notices order by regtime limit start_record, count
            ...
            //$msgs=$dao->getManyMsgs(); //noticesDao의 getManyMsgs()를 $msgs에 담음
        ...
        <?php if($totalCount>0) : ?> 
            ...
            <?php if($startPage>1) : ?>
                <a href='<?=combineUrl("notices.php",0,$currentPage-NUM_PAGE_LINKS)?>' ><</a>&nbsp;
            <?php endif?>

            <?php for($i=$startPage;$i<=$endPage-1;$i++) : ?>
                <?php if($i==$currentPage) : ?>
                    <a href="<?=combineUrl('notices.php',0,$i)?>"><b><?= $i?></b></a>&nbsp;
                <?php else : ?>
                    <a href="<?=combineUrl('notices.php',0,$i)?>"><?= $i ?></a>&nbsp;
                <?php endif ?>
            <?php endfor ?>

            <?php if($endPage<$totalPage) : ?>
                <a href="<?=combineUrl('notices.php',0,$startPage+NUM_PAGE_LINKS)?>">></a>
            <?php endif ?>

        <?php endif ?>
        <input class="btn btn-primary" type="button" value="글쓰기" onclick="location.href='write_form.php?customerWrite=0'">
    </div>
</body>
</html>
