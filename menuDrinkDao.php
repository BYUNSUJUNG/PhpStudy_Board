...
<?php
	class menuDrinkDao {
		...
		function getMsg($num){ 
            try{
                $pstmt = $this->db->prepare("select * from menuDrink where num=:num");
		...
		function insertBoard($writer, $title, $file, $content) { 
			try {
				$sql = "insert into menuDrink(writer,title,file,content) values(:writer,:title,:file,:content)";
		...
		function getManyMsgs($num_page) { 
			...
			try {
				$numLine=NUM_LINES;
				$pstmt=$this->db->prepare("select * from menuDrink order by num desc limit $num_page, $numLine");
				// bindValue 필요 없음
				$pstmt->execute();
		...
		function updateBoard($num, $writer, $title, $file, $content) { 
			try {
				$sql = "update menuDrink set writer=:writer, title=:title, file=:file, content=:content where num=:num";
				$pstmt = $this->db->prepare($sql);
				$pstmt->bindValue(":num",$num,PDO::PARAM_STR);
				$pstmt->bindValue(":writer",$writer,PDO::PARAM_STR);
				$pstmt->bindValue(":title",$title,PDO::PARAM_STR);
		...
		function deleteBoard($num) { 
			try { 
				$sql = "delete from menuDrink where num=:num";
				$pstmt = $this->db->prepare($sql);
				$pstmt->bindValue(":num",$num,PDO::PARAM_STR);
				$pstmt->execute(); // 실행
			} catch(PDOException $e) {
		...
		function getCountMsgs() {
			...
			try { 
				$rows = $this->db->prepare("select count(*) from menuDrink");
				$rows->execute();
