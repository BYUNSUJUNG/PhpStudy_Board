...
<html>
	...
	<body>
		...
		<article>
			...
			onclick="location.href='menuBurger.php'" value="글 목록"> <!-- 글 목록 -->
			<?php if($name==$writer) { ?>
				<input type="button" class="btn btn-danger"
				onclick="location.href='shoppingCart_check.php?menu=Burger&title=<?=$title?>&file=<?=$file?>'" value="담기"> <!-- 담기 -->
				<input type="button" class="btn btn-success"
				onclick="location.href='modify_form.php?num=<?=$num?>&writer=<?=$writer?>&modifyNum=2'" value="글 수정"> <!-- 글 수정 -->
				<input type="button" class="btn btn-danger"
				onclick="location.href='delete_user_check.php?num=<?=$num?>&writer=<?=$writer?>&deleteNum=2'" value="글 삭제"> <!-- 글 삭제 -->
			<?php } ?>
		</article>
