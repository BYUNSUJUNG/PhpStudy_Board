	...
	<body>
		...
		<article>
			...
			<?php if($name==$writer) { ?>
				...
				onclick="location.href='shoppingCart_check.php?menu=Drink&title=<?=$title?>&file=<?=$file?>'" value="담기"> <!-- 담기 -->
				<input type="button" class="btn btn-success"
				onclick="location.href='modify_form.php?num=<?=$num?>&writer=<?=$writer?>&modifyNum=5'" value="글 수정"> <!-- 글 수정 -->
				<input type="button" class="btn btn-danger"
				onclick="location.href='delete_user_check.php?num=<?=$num?>&writer=<?=$writer?>&deleteNum=5'" value="글 삭제"> <!-- 글 삭제 -->
			<?php } ?>
		</article>
